...
exports.run = (client, message, args) => {
  if (message.guild === null) {
    ...
    embed
      .setColor("RANDOM")
      .setAuthor(
        `${client.guilds.get(name).name}`,
        client.guilds.get(name).iconURL
      )
      ...
      .addField(
        `Channels`,
        `${client.guilds.get(name).channels.map(c => c.name).join(", ")}`
      )
      .addField(
        `Roles`,
        `${client.guilds.get(name).roles.map(r => r.name).join(", ")}`
      )
